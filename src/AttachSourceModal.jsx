...
function AttachSourceModal({ allSources, allPeople, onAttach, onCreateNew, onClose, onEditSource }) {
  ...
  return (
    <div className="modal" style={{ display: 'block' }}>
      <div className="modal-content card bg-surface-2 border border-subtle p-6 rounded-xl max-w-3xl text-primary">
        <h3 className="text-xl font-bold mb-4 text-primary">Koppla Källa till Händelser</h3>
        <input
          type="text"
          placeholder="Sök i källkatalogen..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full p-2 border border-subtle bg-background text-primary rounded mb-4"
        />
        <div className="flex gap-6">
          <div className="w-1/2">
            <h4 className="font-semibold mb-2 text-primary">Välj källa</h4>
            <div className="space-y-2 max-h-72 overflow-y-auto border border-subtle bg-surface-2 rounded p-2 mb-4">
              {filteredSources.length === 0 ? (
                <p className="text-muted italic p-4 text-center">Inga källor finns i källkatalogen.</p>
              ) : (
                filteredSources.map(source => {
                  const { yearRange, pageNumber, aid } = getParsedSourceInfo(source);
                  return (
                    <div key={source.id} className={`p-2 rounded flex gap-3 items-center cursor-pointer ${selectedSourceIds.includes(source.id) ? 'bg-accent-soft' : 'hover:bg-surface'}`}>
                      ...
                      <button type="button" className="ml-2 px-2 py-1 text-xs bg-surface text-primary rounded hover:bg-surface-2" onClick={(e) => { e.stopPropagation(); if (onEditSource) onEditSource(source.id); }}>
                        Redigera
                      </button>
                    </div>
                  );
            ...
            <button type="button" onClick={() => onCreateNew(searchTerm)} className="px-4 py-2 bg-accent text-on-accent rounded hover:bg-accent/90">Skapa ny källa...</button>
          </div>
          <div className="w-1/2">
            <h4 className="font-semibold mb-2 text-primary">Välj händelser</h4>
            <div className="space-y-2 max-h-72 overflow-y-auto border border-subtle bg-surface-2 rounded p-2 mb-4">
              {relevantPeople.map(person => (
                <div key={person.id}>
                  <div className="font-bold text-secondary text-xs mb-1">REF: {person.refNumber} {person.firstName} {person.lastName}</div>
        ...
        <div className="flex justify-end items-center mt-6 gap-2">
          <button type="button" onClick={onClose} className="px-4 py-2 border border-subtle rounded hover:bg-surface">Avbryt</button>
          <button
            type="button"
            onClick={() => onAttach(selectedSourceIds, selectedEventIds)}
            className="px-4 py-2 bg-accent text-on-accent rounded hover:bg-accent/90"
