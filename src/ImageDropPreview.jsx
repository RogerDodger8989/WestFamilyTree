...
export default function ImageDropPreview({ source }) {
  ...
  return (
    <div id="image-drop-preview" className="flex flex-col items-center justify-center h-[110px] w-[90px] border-2 border-dashed border-strong rounded bg-surface-2 text-muted text-xs select-none" style={{ cursor: 'pointer' }}>
      ...
      <span className="text-[10px] mt-1 text-muted">Filnamn: {previewName}</span>
      {previewSize && <span className="text-[10px] text-muted">Storlek: {previewSize} kB</span>}
    </div>
  );
}