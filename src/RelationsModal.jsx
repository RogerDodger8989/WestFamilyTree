...
function PersonCard({ person, label, onSelect, onCreateNew, onRemove, people }) {
    if (!person) {
        return (
            <div className="p-4 border-2 border-dashed rounded-lg text-center">
                ...
                <button onClick={onSelect} className="px-3 py-1 text-sm bg-accent-soft text-accent rounded hover:bg-accent-soft">Välj befintlig</button>
    ...
    return (
        <div className="p-4 border rounded-lg bg-background border-subtle shadow-sm relative">
            <div className="text-xs text-secondary">{label}</div>
            <div className="font-bold">{person.firstName} {person.lastName}</div>
            <div className="text-sm text-secondary">{getLifeSpanString(person)}</div>
            {onRemove && (
    ...
    if (isSelecting) {
        return (
            <div className="modal" style={{ display: 'block' }}>
                <div className="modal-content card bg-surface border border-subtle p-6 rounded-xl max-w-lg">
                    ...
                    <ul className="max-h-64 overflow-y-auto">
                        {filteredPeople.map(p => (
                            <li key={p.id} onClick={() => handleSelectPersonForRelation(p.id)} className="p-2 hover:bg-accent-soft cursor-pointer rounded">
                                {p.firstName} {p.lastName} <span className="text-secondary">{getLifeSpanString(p)}</span>
                            </li>
                        ))}
                    </ul>
                    <div className="mt-4 flex justify-end">
                        <button onClick={() => setIsSelecting(null)} className="px-4 py-2 border border-subtle rounded hover:bg-surface-2 text-primary">Avbryt</button>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="modal" style={{ display: 'block' }}>
            ...
            {editingRelationId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                    <div className="bg-surface border border-subtle rounded-xl p-6 w-full max-w-2xl">
                        <h4 className="text-lg font-bold mb-3">Redigera relation</h4>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-xs text-secondary">Roll (från)</label>
                            ...
                            <div>
                                <label className="block text-xs text-secondary">Roll (till)</label>
                            ...
                            <div>
                                <label className="block text-xs text-secondary">Startdatum</label>
                            ...
                            <div>
                                <label className="block text-xs text-secondary">Slutdatum</label>
                            ...
                            <div>
                                <label className="block text-xs text-secondary">Säkerhet</label>
                            ...
                            <div>
                                <label className="block text-xs text-secondary">Orsak / Notering</label>
                                <input value={editingRelationData.reason || ''} onChange={e => setEditingRelationData(prev => ({ ...prev, reason: e.target.value }))} className="w-full p-2 border rounded" />
                                                ...
                                                <div className="mt-3">
                                                        <label className="block text-xs text-secondary">Anteckning</label>
                                                        <Editor
            ...
            <div className="modal-content card bg-surface border border-subtle p-6 rounded-xl max-w-4xl">
                <h3 className="text-2xl font-bold mb-6 text-center">Familjeöversikt för {person.firstName} {person.lastName}</h3>

                {/* Föräldrar */}
                <div className="mb-6">
                    <h4 className="font-semibold text-lg mb-2 text-primary">Föräldrar</h4>
                ...
                {/* Fokusperson & Partner */}
                <div className="mb-6 p-4 bg-accent-soft rounded-lg">
                     <h4 className="font-semibold text-lg mb-2 text-primary">Fokusperson & Partner</h4>
                     <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 border rounded-lg bg-background border-subtle shadow-sm ring-2 ring-blue-500">
                            <div className="text-xs text-accent font-bold">Fokusperson</div>
                            <div className="font-bold text-xl">{person.firstName} {person.lastName}</div>
                            <div className="text-md text-secondary">{getLifeSpanString(person)}</div>
                        </div>
                ...
                {/* Barn */}
                <div>
                    <h4 className="font-semibold text-lg mb-2 text-primary">Barn</h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <div className="p-4 border-2 border-dashed rounded-lg text-center flex flex-col justify-center items-center">
                             <button onClick={() => setIsSelecting('child')} className="px-3 py-1 text-sm bg-accent-soft text-accent rounded hover:bg-accent-soft">Välj befintligt barn</button>
                ...
                <div className="mt-8 flex justify-end gap-2">
                    <button type="button" onClick={onClose} className="px-4 py-2 border border-subtle rounded hover:bg-surface-2 text-primary">Avbryt</button>
                    <button type="button" onClick={handleSave} className="px-6 py-2 bg-accent text-on-accent rounded font-bold hover:bg-accent">Spara Relationer</button>
                </div>
            </div>
        </div>
    );
