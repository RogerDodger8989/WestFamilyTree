...
export default function PlaceConnections({ place, allPeople, onPersonClick }) {
    ...
    return (
        <div className="mt-6">
            <h4 className="text-lg font-semibold text-primary border-b pb-2 mb-2">Personer & Händelser på denna plats</h4>
            <ul className="space-y-2">
                {connectedPeople.map(({ person, event }, index) => (
                    <li key={`${person.id}-${event.id}-${index}`} className="text-sm p-2 bg-surface rounded-md border border-subtle">
                        ...
                        <span className="text-secondary"> - {event.type} ({event.date || 'Okänt datum'})</span>
                    </li>
                ))}
            </ul>
        </div>
