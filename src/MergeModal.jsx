...
export default function MergeModal({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-background/40">
            <div className="bg-surface rounded-lg p-6 w-full max-w-3xl border border-subtle">
                <h3 className="text-xl font-bold mb-3">Sammanslå personer</h3>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <div className="text-sm text-primary mb-2">Målperson</div>
                    ...
                    <div>
                        <div className="text-sm text-primary mb-2">Källor att slå ihop (välj flera)</div>
                ...
                <div className="mt-4">
                    ...
                    {!preview && <div className="text-xs text-muted">Välj mål och källor för att se vad som flyttas.</div>}
                    {preview && (
                        <div className="mt-2 text-sm">
                            ...
                            <div className="mt-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-xs text-secondary">{(preview.keptList || []).length}</div>
                                </div>
                                <div className="mt-1">
                                    <button onClick={() => setKeptOpen(s => !s)} className="text-xs text-accent mb-2">{keptOpen ? 'Dölj' : 'Visa'}</button>
                                    {keptOpen && (
                                        <>
                                            {(preview.keptList || []).length === 0 && <div className="text-xs text-muted">Inga relationer kommer att behållas.</div>}
                                            {(preview.keptList || []).length > 0 && (
                                                <>
                                                    <ul className="mt-1 list-disc pl-5 text-xs">
                                                        {((keptShowAll ? preview.keptList : preview.keptList.slice(0, 8)) || []).map(r => (
                                                            <li key={r.id}>{r.type || 'relation'}: {r.from} → {r.to} <span className="text-muted">({r.id})</span></li>
                                                        ))}
                                                    </ul>
                                                    {preview.keptList.length > 8 && (
                                                        <div className="mt-1">
                                                            <button onClick={() => setKeptShowAll(s => !s)} className="text-xs text-accent">{keptShowAll ? 'Visa färre' : `Visa fler (${preview.keptList.length - 8} fler)`}</button>
                                                        </div>
                                                    )}
                                                </>
                                            )}
                            ...
                            <div className="mt-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-xs text-secondary">{(preview.archivedList || []).length}</div>
                                </div>
                                <div className="mt-1">
                                    <button onClick={() => setArchivedOpen(s => !s)} className="text-xs text-accent mb-2">{archivedOpen ? 'Dölj' : 'Visa'}</button>
                                    {archivedOpen && (
                                        <>
                                            {(preview.archivedList || []).length === 0 && <div className="text-xs text-muted">Inga relationer kommer att arkiveras.</div>}
                                            {(preview.archivedList || []).length > 0 && (
                                                <>
                                                    <ul className="mt-1 list-disc pl-5 text-xs">
                                                        {((archivedShowAll ? preview.archivedList : preview.archivedList.slice(0, 8)) || []).map(r => (
                                                            <li key={r.id}>{r.type || 'relation'}: {r.from} → {r.to} - <em>{r.reason}</em> <span className="text-muted">({r.id})</span></li>
                                                        ))}
                                                    </ul>
                                                    {preview.archivedList.length > 8 && (
                                                        <div className="mt-1">
                                                            <button onClick={() => setArchivedShowAll(s => !s)} className="text-xs text-accent">{archivedShowAll ? 'Visa färre' : `Visa fler (${preview.archivedList.length - 8} fler)`}</button>
                                                        </div>
                                                    )}
                                                </>
                                            )}
