...
function PersonCard({ person, label, onSelect, onCreateNew, onRemove, people }) {
    ...
    return (
        <div className="p-4 border rounded-lg bg-white shadow-sm relative">
            ...
            <div className="text-sm text-gray-600">{getLifeSpanString(person)}</div>
            {onRemove && (
...
export default function RelationsModal({ person, allPeople, onClose, onCreateNewPerson }) {
    ...
    if (isSelecting) {
        return (
            <div className="modal" style={{ display: 'block' }}>
                <div className="modal-content card bg-white p-6 rounded-xl max-w-lg">
                    ...
                    <ul className="max-h-64 overflow-y-auto">
                        {filteredPeople.map(p => (
                            <li key={p.id} onClick={() => handleSelectPersonForRelation(p.id)} className="p-2 hover:bg-blue-100 cursor-pointer rounded">
                                {p.firstName} {p.lastName} <span className="text-gray-500">{getLifeSpanString(p)}</span>
                            </li>
                        ))}
                    </ul>
                    <div className="mt-4 flex justify-end">
                        <button onClick={() => setIsSelecting(null)} className="px-4 py-2 border rounded hover:bg-gray-50">Avbryt</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="modal" style={{ display: 'block' }}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                    <div className="bg-white rounded-xl p-6 w-full max-w-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-xs text-gray-600">Roll (från)</label>
                            ...
                            <div>
                                <label className="block text-xs text-gray-600">Roll (till)</label>
                            ...
                            <div>
                                <label className="block text-xs text-gray-600">Startdatum</label>
                            ...
                            <div>
                                <label className="block text-xs text-gray-600">Slutdatum</label>
                            ...
                            <div>
                                <label className="block text-xs text-gray-600">Säkerhet</label>
                            ...
                            <div>
                                <label className="block text-xs text-gray-600">Orsak / Notering</label>
                                <input value={editingRelationData.reason || ''} onChange={e => setEditingRelationData(prev => ({ ...prev, reason: e.target.value }))} className="w-full p-2 border rounded" />
                                                ...
                                                <div className="mt-3">
                                                        <label className="block text-xs text-gray-600">Anteckning</label>
                                                        <Editor
            ...
            <div className="modal-content card bg-white p-6 rounded-xl max-w-4xl">
                <h3 className="text-2xl font-bold mb-6 text-center">Familjeöversikt för {person.firstName} {person.lastName}</h3>

                {/* Föräldrar */}
                <div className="mb-6">
                    <h4 className="font-semibold text-lg mb-2 text-gray-700">Föräldrar</h4>
                ...
                <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                     <h4 className="font-semibold text-lg mb-2 text-gray-700">Fokusperson & Partner</h4>
                     <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 border rounded-lg bg-white shadow-sm ring-2 ring-blue-500">
                            ...
                            <div className="text-md text-gray-600">{getLifeSpanString(person)}</div>
                        </div>
                ...
                {/* Barn */}
                <div>
                    <h4 className="font-semibold text-lg mb-2 text-gray-700">Barn</h4>
