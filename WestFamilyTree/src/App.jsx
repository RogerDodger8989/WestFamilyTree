...
function App() {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      try { if (newDbResult) { if (editingPerson && editingPerson.id) { const updatedPerson = newDbResult.people.find(p => p.id === editingPerson.id); if (updatedPerson) { handleEditFormChange(JSON.parse(JSON.stringify(updatedPerson))); } } if (personDrawer && personDrawer.id) { const updatedDrawerPerson = newDbResult.people.find(p => p.id === personDrawer.id); if (updatedDrawerPerson) { handleEditFormChange(JSON.parse(JSON.stringify(updatedDrawerPerson))); } } } } catch (err) { }
    };
    ...
    <div className="h-screen flex flex-col" style={{ overflow: 'hidden' }}>
      ...
      <div className="menubar shadow-md shrink-0">
        <div className="flex items-center space-x-4">
          ...
          <Button onClick={handleNewFile} variant="secondary" size="sm">Ny</Button>
          ...
          <Button onClick={handleSaveFileAs} variant="secondary" size="sm">Spara som...</Button>
