...
export default function ImageGallery({ source, onEditSource, people, onOpenEditModal }) {
  ...
  return (
    <div className="flex h-full gap-4 outline-none" onPaste={onPaste} tabIndex="0"> 
      ...
      <div className={`flex-1 border-2 border-dashed rounded-lg overflow-y-auto p-4 transition-colors relative min-h-[300px] ${isDragging ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'}`}
          onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
      >
          {images.length === 0 && !uploading && (<div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 pointer-events-none"><p className="text-lg font-semibold">Dra och släpp bilder här</p><p className="text-sm">eller klistra in (Ctrl+V)</p></div>)}
          {uploading && (<div className="absolute inset-0 flex items-center justify-center bg-white/80 z-10"><p className="text-blue-600 font-bold animate-pulse">Sparar bilder...</p></div>)}
            ...
            {images.map((img, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center gap-2">
                  <div onClick={() => setSelectedImageIndex(idx)} onDoubleClick={openInternalViewer} className={`cursor-pointer rounded border-2 overflow-hidden relative group bg-white shadow-sm aspect-square w-32 h-32 flex-shrink-0 flex items-center justify-center ${selectedImageIndex === idx ? 'border-blue-600 ring-2 ring-blue-200' : 'border-transparent hover:border-gray-400'}`}>
                    {previews[img.src] ? (
                      <img src={previews[img.src]} alt={typeof img.title === 'string' ? img.title : ''} className="w-full h-full object-cover" />
                    ) : (
                      <div className="text-gray-400 text-xs p-2 text-center break-all">
                  ...
                  <div className="flex flex-wrap gap-1 items-center min-h-[2rem]">
                    ...
                    )) : <span className="text-gray-400 text-xs italic">Ingen person taggad</span>}
                  </div>
                </div>
              );
            })}
            <label className="cursor-pointer border-2 border-dashed border-gray-300 rounded flex flex-col items-center justify-center aspect-square w-32 h-32 hover:bg-white hover:border-blue-400 transition-colors bg-gray-50 mt-2">
              <span className="text-3xl text-gray-300 mb-1">+</span>
              <span className="text-xs text-gray-500 font-medium">+ lägg till</span>
              <span className="text-xs text-gray-400 mt-1">ctrl+v</span>
      ...
      {selectedImg ? (
          <div className="w-1/3 bg-white border-l pl-4 flex flex-col gap-3 overflow-y-auto">
              <h3 className="font-bold text-gray-700 text-sm border-b pb-2">Bildinformation</h3>
              <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Titel</label><input className="w-full border rounded px-2 py-1 text-sm" value={selectedImg.title || ''} onChange={(e) => handleImageUpdate('title', e.target.value)} /></div>
              <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Datum</label>
                  <input className="w-full border rounded px-2 py-1 text-sm" placeholder="ÅÅÅÅ-MM-DD" value={selectedImg.date || ''} 
                    onChange={(e) => handleImageUpdate('date', e.target.value)}
                    onBlur={handleDateBlur} 
                  />
              </div>
              <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Taggar</label><TagInput value={selectedImg.tags || ''} onChange={(newValue) => handleImageUpdate('tags', newValue)} placeholder="Semester, Bröllop..." /></div>
              <div className="flex-1 min-h-[100px]"><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Notering</label><textarea className="w-full border rounded px-2 py-1 text-sm h-full resize-none bg-gray-50 focus:bg-white" value={selectedImg.note || ''} onChange={(e) => handleImageUpdate('note', e.target.value)} /></div>
              {/* Visa taggade personer */}
              {Array.isArray(selectedImg.regions) && selectedImg.regions.length > 0 && (
                <div className="mt-2">
                  <div className="text-xs font-bold text-gray-500 uppercase mb-1">Taggade personer</div>
                  <ul className="space-y-1">
                    {selectedImg.regions.map((region, idx) => {
                      const person = people?.find(p => p.id === region.personId);
                      return person ? (
                        <li key={region.personId + '-' + idx} className="text-sm text-gray-700 flex items-center gap-2">
                          <span className="font-semibold">{person.firstName} {person.lastName}</span>
                          <span className="text-xs text-gray-400">(REF: {person.refNumber})</span>
                        </li>
                      ) : null;
                    })}
                  </ul>
      ...
      ) : (<div className="w-1/3 flex items-center justify-center text-gray-400 text-xs italic border-l bg-gray-50">Välj en bild för att redigera detaljer</div>)}
    </div>
  );
}