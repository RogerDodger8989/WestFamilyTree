...
export default function PlacePicker({ value, allPlaces, onChange }) {
    ...
    const filteredPlaces = useMemo(() => {
        ...
        if (Array.isArray(serverResults) && serverResults.length > 0) {
            // Deduplicate by id and name+hierarchy to avoid visible duplicates
            const seen = new Set();
            const deduped = [];
            for (const p of serverResults) {
                const key = `${p.id}::${p.name}::${p.kommunnamn || ''}::${p.lansnamn || ''}::${p.sockenstadnamn || ''}`;
    ...
    const renderItem = (place) => {
        ...
        const type = (raw.detaljtyp || raw.type || '').toLowerCase();
        const metaParts = [];
        ...
        const typeLabel = type === 'parish' ? 'Församling' : type === 'municipality' ? 'Kommun' : type === 'county' ? 'Län' : type === 'village' ? 'Ort' : type === 'building' ? 'Byggnad' : type === 'cemetary' ? 'Kyrkogård' : '';
        return (
