        ...
        <div className="fixed inset-0 z-60 flex items-start justify-center bg-black bg-opacity-40 p-6">
            <div className="bg-white rounded-lg p-4 w-full max-w-2xl">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => onClose && onClose()} className="text-gray-500">Stäng</button>
                </div>
                <div className="max-h-72 overflow-y-auto text-sm">
                    {merges.length === 0 && <div className="text-gray-500">Inga merges registrerade.</div>}
                    {merges.map(m => {
                            ...
                            <div key={m.id} className="p-3 border-b last:border-b-0">
                                <div className="flex justify-between items-start gap-3">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-600">Mergade in i: {m.mergedIntoId} {mergedIntoPerson ? `(${mergedIntoPerson.firstName} ${mergedIntoPerson.lastName})` : ''} | {m.originalPersonIds.join(', ')}</div>
                                        <div className="text-xs text-gray-500 mt-1">{new Date(m.createdAt).toLocaleString()} av {m.createdBy || 'system'}</div>
                                    </div>
                                ...
                                {pendingUndoId === m.id && (
                                    <div className="mt-3 bg-yellow-50 p-3 rounded">
                                        <div className="text-sm text-gray-700 mb-2">Skriv REF-nummer för målpersonen för att bekräfta återställning</div>
                                        ...
                                        {expectedRef && <div className="text-xs text-gray-500 mt-2">Förväntat REF: <strong>{expectedRef}</strong></div>}
                                    </div>
                                )}
                            </div>
                        );
