    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded shadow-lg p-6 w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <select className="w-full border rounded p-1 text-sm bg-gray-50" value={country} onChange={e => setCountry(e.target.value)} disabled>
              ...
              {codeFields.map(field => field.value && (
                <div key={field.label} className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="font-mono bg-white border px-2 py-0.5 rounded text-gray-800 min-w-[3rem] text-center">
                      {field.value}
                    </span>
                    <button
        ...
        {!selectedPlace && (
          <div className="mt-6 flex flex-col items-center">
            <div className="text-sm text-gray-600 mb-2">Ingen officiell plats hittades.</div>
            <button
        ...
        <div className="flex justify-end gap-2 mt-6 border-t pt-4">
          <button onClick={onClose} className="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded text-sm font-medium">Avbryt</button>
          <button
            onClick={() => selectedPlace && onLink(selectedPlace)}
            className={`px-4 py-2 rounded text-white text-sm font-bold shadow-sm ${selectedPlace ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-400 cursor-not-allowed'}`}
            disabled={!selectedPlace}
          >
            Koppla vald plats
          </button>
        </div>
        {loading && <div className="text-xs text-gray-500 mt-2 text-center">Laddar...</div>}
      </div>
    </div>
  );
}