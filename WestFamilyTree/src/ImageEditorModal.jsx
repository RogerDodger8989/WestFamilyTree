...
import { RotateCw, ZoomIn, ZoomOut, Maximize2, Save, Image as ImageIcon, ArrowLeft, ArrowRight, XCircle, Trash2, Crop, Repeat, Sun, Contrast, FlipHorizontal, FlipVertical, Grid, RotateCcw } from 'lucide-react';
...
const ImageEditorModal = ({
  ...
  useEffect(() => {
    ...
    const handleKey = (e) => {
      ...
      if (e.key.toLowerCase() === 'r') { e.preventDefault(); applyRotation(90); }
      if (e.key.toLowerCase() === 'f') { e.preventDefault(); handleFitToWindow(); }
      if (e.key.toLowerCase() === 'z') { e.preventDefault(); handleZoomIn(); }
      if (e.key.toLowerCase() === 'x') { e.preventDefault(); handleZoomOut(); }
      if (e.key.toLowerCase() === 'c') { e.preventDefault(); toggleCropMode(); }
      if (e.key === 'Delete' && onDelete) { e.preventDefault(); onDelete(); }
      if (e.key.toLowerCase() === 'h') { e.preventDefault(); resetTransforms(); }
  ...
  }, [isOpen, hasPrev, hasNext, onPrev, onNext, onClose, onDelete, applyRotation, handleFitToWindow, handleZoomIn, handleZoomOut, toggleCropMode, resetTransforms, handleUndo, handleRedo]);
      ...
      <div className="flex flex-col h-full bg-slate-900 select-none">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-700 bg-slate-800 shrink-0 select-none text-slate-300 text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onDelete}
              className={`flex items-center gap-2 px-3 py-2 rounded transition-colors font-medium ${isConfirmingDelete ? 'bg-red-700 hover:bg-red-800' : 'bg-amber-700 hover:bg-amber-800'} text-white`}
              title={isConfirmingDelete ? 'Bekräfta radering' : 'Ta bort bild'}
            >
              <Trash2 size={16} />
              {isConfirmingDelete ? 'Bekräfta' : 'Ta bort'}
            </button>
            <button
              onClick={onClose}
              className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition-colors font-medium"
              title="Avbryt"
            >
              <XCircle size={16} />
              Avbryt
            ...
            {enableSaveCopy && (
              <label className="flex items-center gap-2 text-xs text-slate-200">
                <input type="checkbox" checked={saveAsCopy} onChange={(e) => setSaveAsCopy(e.target.checked)} />
                Spara som kopia
              </label>
            )}
          </div>
        ...
        <div 
          ...
          style={{ cursor: isCropMode ? 'crosshair' : 'move' }}
          onWheel={handleWheel}
