...
const GedcomImporter = (props) => {
  ...
  if (!data) {
    return (
      <div className="p-10 border-2 border-dashed text-center rounded-lg bg-gray-50 m-4">
        <h2 className="text-xl font-bold mb-4">Ladda upp GEDCOM</h2>
        <input type="file" accept=".ged" onChange={handleFileUpload} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
    ...
    <div className="p-4 h-screen flex flex-col">
      <div className="flex justify-between items-center mb-4 border-b pb-4">
         ...
         <div className="flex gap-3">
             <button onClick={() => setData(null)} className="px-4 py-2 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">Ny fil</button>
      ...
      <div className="flex gap-4 flex-1 overflow-hidden">
        <div className="w-1/3 bg-white shadow rounded p-4 overflow-y-auto">
          <h3 className="font-bold border-b pb-2 sticky top-0 bg-white">Personer</h3>
          <ul className="mt-2">
            {data.individuals.slice(0, 50).map(indi => (
              <li key={indi.id} className="py-2 border-b hover:bg-gray-50 text-sm">
                <div className="font-semibold">{indi.name}</div>
        ...
        <div className="w-2/3 bg-gray-50 p-4 text-center text-gray-500">
