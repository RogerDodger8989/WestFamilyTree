        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40">
            <div className="bg-white rounded-lg p-6 w-full max-w-3xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <div className="text-sm text-gray-600 mb-2">Målperson</div>
                    ...
                    <div>
                        <div className="text-sm text-gray-600 mb-2">Källor att slå ihop (välj flera)</div>
                ...
                <div className="mt-4">
                    ...
                    {!preview && <div className="text-xs text-gray-500">Välj mål och källor för att se vad som flyttas.</div>}
                    {preview && (
                        <div className="mt-2 text-sm">
                            ...
                            <div className="mt-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-xs text-gray-500">{(preview.keptList || []).length}</div>
                                </div>
                                <div className="mt-1">
                                    <button onClick={() => setKeptOpen(s => !s)} className="text-xs text-blue-600 mb-2">{keptOpen ? 'Dölj' : 'Visa'}</button>
                                    {keptOpen && (
                                        <>
                                            {(preview.keptList || []).length === 0 && <div className="text-xs text-gray-500">Inga relationer kommer att behållas.</div>}
                                            {(preview.keptList || []).length > 0 && (
                                                <>
                                                    <ul className="mt-1 list-disc pl-5 text-xs">
                                                        {((keptShowAll ? preview.keptList : preview.keptList.slice(0, 8)) || []).map(r => (
                                                            <li key={r.id}>{r.type || 'relation'}: {r.from} → {r.to} <span className="text-gray-500">({r.id})</span></li>
                                                        ))}
                                                    </ul>
                            ...
                            <div className="mt-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-xs text-gray-500">{(preview.archivedList || []).length}</div>
                                </div>
                                <div className="mt-1">
                                    <button onClick={() => setArchivedOpen(s => !s)} className="text-xs text-blue-600 mb-2">{archivedOpen ? 'Dölj' : 'Visa'}</button>
                                    {archivedOpen && (
                                        <>
                                            {(preview.archivedList || []).length === 0 && <div className="text-xs text-gray-500">Inga relationer kommer att arkiveras.</div>}
                                            {(preview.archivedList || []).length > 0 && (
                                                <>
                                                    <ul className="mt-1 list-disc pl-5 text-xs">
                                                        {((archivedShowAll ? preview.archivedList : preview.archivedList.slice(0, 8)) || []).map(r => (
                                                            <li key={r.id}>{r.type || 'relation'}: {r.from} → {r.to} — <em>{r.reason}</em> <span className="text-gray-500">({r.id})</span></li>
                                                        ))}
                                                    </ul>
